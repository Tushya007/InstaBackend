...
def signup(request):
    ...
    hashed_password = bcrypt.hashpw(
        serializer.data['password'].encode(), bcrypt.gensalt())
    user = User.objects.create(
        username=serializer.data['username'], email=serializer.data['email'], password=hashed_password)
    if user is not None:
...
def login(request):
    ...
    user_by_username = User.objects.filter(
        username=serialzer.data['username']).first()
